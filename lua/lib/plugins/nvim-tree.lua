...
function M.cd()
   ...
   -- ──────────────────────────────────────────────────────────────────────
   local node = to_require_map["nvim-tree.lua"]["nvim-tree.lib"].get_node_at_cursor()
   local actions = to_require_map["nvim-tree.lua"]["nvim-tree.actions"]
   if node.has_children then
      local result = vim.fn.chdir(node.absolute_path)
      if result == "" then
         error("failed to cd to " .. node.absolute_path, vim.diagnostic.severity.E)
         return
      end
   end
   actions.on_keypress("cd")
end
